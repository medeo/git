...
module.exports = async (
  ...
  if (isUndefined(assets) && (await pathExists(path.resolve(cwd, CHANGELOG)))) {
    logger.log('Add %s to the release commit', CHANGELOG);
    patterns.push(CHANGELOG);
  }
  if (isUndefined(assets) && (await pathExists(path.resolve(cwd, PKG_JSON)))) {
    logger.log('Add %s to the release commit', PKG_JSON);
    patterns.push(PKG_JSON);
  }
  if (isUndefined(assets) && (await pathExists(path.resolve(cwd, PKG_LOCK)))) {
    logger.log('Add %s to the release commit', PKG_LOCK);
    patterns.push(PKG_LOCK);
  }
  if (isUndefined(assets) && (await pathExists(path.resolve(cwd, SKW_JSON)))) {
