...
const APP_PKG_JSON = path.join('app', 'package.json');
...
module.exports = async (pluginConfig, {options: {branch, repositoryUrl}, lastRelease, nextRelease, logger}) => {
  ...
  if (isUndefined(assets) && (await pathExists(APP_PKG_JSON))) {
    logger.log('Add %s to the release commit', APP_PKG_JSON);
    patterns.push(APP_PKG_JSON);
  }
